...
Controller.prototype.handleRequest = function (request, response, next, settings) {
  ...
  if (!request.parsedUrl) {
    ...
    proto = request.headers['x-forwarded-proto'] || request.headers['x-scheme'] || proto;
    proto = proto.split(/\s*,\s*/)[0];

    var parsedUrl = _.defaults(_.pick(url.parse(request.url, true), 'path', 'pathname', 'query'),
        this._baseUrl, { protocol: proto + ':', host: request.headers.host });
    // var scriptName = request.headers['x-script-name'];
    // if (scriptName && parsedUrl.pathname.startsWith(scriptName))
    //  parsedUrl.pathname = parsedUrl.pathname.substr(scriptName.length);

    request.parsedUrl = parsedUrl;
  }

  // Since @base prefix depends on request URL we copy prefixes to request.prefixes
  if (!request.prefixes) {
    var baseRequestUrl = _.omit(request.parsedUrl, 'search', 'query', 'pathname');
    var basePathname = ''; // requestUrl.pathname;
    baseRequestUrl.pathname = basePathname + (basePathname.endsWith('/') ? '' : '/');
    var base = url.format(baseRequestUrl);
    var prefixes = _.omit(this._prefixes);
    prefixes[''] = base;
